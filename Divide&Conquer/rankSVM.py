...
class RankSVM(svm.LinearSVC):
    ...
    def predict(self, X):
        ...
        ord : array, shape (n_samples,)
            ...
            the rows in X.
        """
        # we did some modifications here since the argsort did not work
        # correctly without flattening into an array the result from the
        # svm first
        if hasattr(self, 'coef_'):
            check = np.squeeze(np.dot(X, self.coef_.T))
            # reverse order
            return np.argsort(check)[::-1]
        else:
