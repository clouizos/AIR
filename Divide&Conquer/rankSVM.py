...
class RankSVM(svm.LinearSVC):
    ...
    def __init__(self, dual=False):
        super(RankSVM, self).__init__(dual=dual)

    # below is for an SVC(Linear SVC scales better to larger datasets)
    ...
    def predict(self, X):
        ...
        if hasattr(self, 'coef_'):
            #print np.dot(X, self.coef_.T)
            return np.argsort(np.squeeze(np.dot(X, self.coef_.T)))
        else:
