...
# real assignment of query-document features
def gen_q_doc_features(q_doc, V, groups, nr_docs):
    q_doc_features = {}
    split = int(nr_docs/groups)
    cnt = 1
    cnt2 = 0
    for q in q_doc:
        ...
        if cnt2 == cnt * split:
            cnt += 1
        for i in xrange(len(q_docs)):
            q_doc_features[q+':'+q_docs[i]] = np.random.normal(cnt, 1., V)
        cnt2 += 1
...
def gen_user_query(N, queries):
    ...
    for i in range(N):
        ...
        user_q['user_%i' % i] = natsorted(list(set(q_set)))
    return user_q
...
def ass_query_doc(queries, docs, groups):
    ...
    for q in natsorted(queries):
        ...
        docs_assign = np.random.choice(natsorted(docs)[(cnt2 - 1) * split:cnt2 * split], size_rel)
        # remove duplicates
        q_doc[q] = natsorted(list(set(docs_assign)))
        cnt += 1
    return q_doc
...
def gen_dummy(nr_docs, nr_queries, N, groups):
    ...
    q_doc = ass_query_doc(queries, doc_ids, groups)
    # generate query-document features
    q_doc_features = gen_q_doc_features(q_doc, V, groups, nr_docs)
    # randomly generate assignments of clicks per query
    user_clicks = gen_clicks_per_user_query(user_q, q_doc)

    return docs, queries, user_q, q_doc, user_clicks, q_doc_features
